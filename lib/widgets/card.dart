...
class CardTemplate extends StatefulWidget {
  ...
  final double ratio;
  const CardTemplate({Key key, @required this.child, @required this.onTap, @required this.ratio}) : super(key: key);
...
class _CardState extends State<CardTemplate> {
  ...
  Widget build(BuildContext context) {
    return Padding(
        ...
        child: Container(
            height: UtilsImpl.getScreenHeight(context, true) * ((widget.ratio<1.8) ? 0.35 : 0.30),
            child: InkWell(
                onTap: widget.onTap,
                child: Card(
